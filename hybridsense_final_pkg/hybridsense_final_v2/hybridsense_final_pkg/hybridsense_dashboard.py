...
def compute_metrics():
    ...
    # Time-based split: kronolojik sıra korunuyor (veri sızıntısı yok)
    ...
    X = sub[FEATURES]; y = sub["y"].values
    X_te_s = sc.transform(X_te)
    yp_rf = rf.predict(X_te); p_rf = rf.predict_proba(X_te)[:,1]
    yp_lr = lr.predict(X_te_s); p_lr = lr.predict_proba(X_te_s)[:,1]
    ...
    time_set = {"is_workday","is_workhour","is_morning","is_evening","is_night",
...
with tab1:
    ...
    with col_right:
        ...
        fig_cm = go.Figure(go.Heatmap(
            z=CM, x=["Pred: Empty", "Pred: Occupied"],
        ...
        fig_cm.add_annotation(
            x="Pred: Empty", y="Act: Occupied",
            text=f"FN = {CM[1][0]}", showarrow=False,
            font=dict(color="#3FB950" if CM[1][0]==0 else "#D29922", size=13), yshift=-36
        )
    ...
    fig_hr.update_layout(
        **PLOTLY_LAYOUT, height=220,
        xaxis=dict(gridcolor="#21262D", title="Hour of Day", color="#8B949E",
...
with tab2:
    ...
    for col, s in zip([e1, e2, e3, e4], ENERGY):
        ...
        <div style="background:{bg};border:1px solid {border};border-radius:10px;
            ...
            <div style="font-size:26px;font-weight:700;color:#58A6FF;
                        font-family:'JetBrains Mono',monospace;">{s['total']:,}</div>
    ...
    with c_chart:
        ...
        fig_e.add_trace(go.Scatter(
            x=DAILY["date"], y=DAILY["baseline"],
            name="Baseline", line=dict(color="#F85149", width=1.5),
            fill="tozeroy", fillcolor="rgba(248,81,73,0.1)"
        ))
        fig_e.add_trace(go.Scatter(
            x=DAILY["date"], y=DAILY["setback"],
            name="2°C Setback", line=dict(color="#3FB950", width=1.5),
...
with tab3:
    ...
    with col_pmv:
        ...
        for p in PMV_DATA:
            ...
            <div style="background:{bg};border:1px solid {border};border-radius:8px;
                ...
                <div style="font-size:22px;font-weight:700;color:{pmv_col};
                            font-family:'JetBrains Mono',monospace;min-width:70px;">{pmv_str}</div>
                <div style="flex:1;">
                    <div style="font-size:13px;font-weight:600;color:#E6EDF3;">{p['name']}</div>
                    <div style="font-size:11px;color:#8B949E;">
                        met={p['met']} · PPD={p['ppd']}% · Comfort={p['comfort']}%
                    </div>
                </div>
                <span style="background:{badge_col}22;color:{badge_col};
                    border:1px solid {badge_col}44;border-radius:20px;
    ...
    with col_gauge:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            gauge={
                "axis": {"range": [-3, 3], "tickcolor": "#8B949E",
            ...
            title={"text": "PMV Value<br><span style='font-size:11px;color:#8B949E'>Comfort zone: −0.5 to +0.5 (ISO 7730)</span>",
...
with tab4:
    st.markdown("""
    <div class="info-box">
        <span style="display:inline-block;width:8px;height:8px;background:#3FB950;
    ...
    for h in SIM_HOURS:
        ...
        SIM.append({"hour": h, "label": f"{h:02d}:00", "occ": is_occ,
                    "sp": sp_now, "temp": round(t_mean, 2), "pmv": pmv_val})
    ...
    if run:
        ...
        for i, row in enumerate(SIM):
            ...
            if len(temps) > 1:
                ...
                fig_live.add_trace(go.Scatter(
                    x=labels, y=temps, name="Room Temp",
                    line=dict(color="#58A6FF", width=2.5), mode="lines"
                ))
                fig_live.add_trace(go.Scatter(
                    x=labels, y=sps, name=f"Setpoint ({setback})",
                    line=dict(color="#3FB950", width=2, dash="dash"), mode="lines"
                ))
                if show_base:
                    fig_live.add_trace(go.Scatter(
                        x=labels, y=sps_base, name="Baseline SP",
                        line=dict(color="#F85149", width=1, dash="dot"), mode="lines"
                    ))
                fig_live.update_layout(
                    **PLOTLY_LAYOUT, height=260,
                    xaxis=dict(gridcolor="#21262D", color="#8B949E"),
                    yaxis=dict(gridcolor="#21262D", title="Temperature (°C)",
                               color="#8B949E", range=[17, 25]),
                    legend=dict(bgcolor="#21262D", bordercolor="#30363D",
            ...
            <div style="background:#161B22;border:1px solid #30363D;border-radius:8px;
                        padding:12px;font-family:'JetBrains Mono',monospace;min-height:260px;">
        ...
        chart_box.markdown("""
        <div style="height:260px;background:#161B22;border:1px dashed #30363D;border-radius:10px;
                    display:flex;align-items:center;justify-content:center;
                    color:#484F58;font-size:14px;flex-direction:column;gap:8px;">
