...
try:
    ...
    USE_PTC = True
    print("[PMV] pythermalcomfort yüklendi")
except ImportError:
    USE_PTC = False
    print("[PMV] pythermalcomfort yok")
...
def calc_pmv_ppd(tdb, tr, vr, rh, met, clo):
    if USE_PTC:
        r = _ptc_pmv_ppd(tdb=tdb, tr=tr, v=vr, rh=rh, met=met, clo=clo, standard='ISO')
        if isinstance(r, dict):
            return float(r['pmv']), float(r['ppd'])
        return float(r.pmv), float(r.ppd)
    return _pmv_ppd_fallback(tdb, tr, vr, rh, met, clo)

def hesapla_pmv_serisi(t_arr, rh_arr):
    pmv, ppd = [], []
    for t, r in zip(t_arr, rh_arr):
        if pd.isna(t) or pd.isna(r):
            pmv.append(np.nan)
            ppd.append(np.nan)
        else:
            p, d = calc_pmv_ppd(t, t, V_AIR, r, MET, CLO)
            pmv.append(p)
            ppd.append(d)
    return np.array(pmv), np.array(ppd)
...
for sb in SETBACKS:
    ...
    lbl = "Baseline" if sb == 0 else f"Setback {sb}"
...
out_fig = os.path.join(OUTPUT_DIR, "pmv.png")
plt.figure()
plt.plot(df["pmv_baseline"])
plt.savefig(out_fig)
plt.close()

print("Kaydedildi:", out_fig)
